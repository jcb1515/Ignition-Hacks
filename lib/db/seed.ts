...
import { insertTransaction, insertVendor } from "./queries";
import type { Source, Transaction, Vendor } from "../types";
...
/**
 * Wipes and reseeds the database. Safe to call repeatedly in one process: the
 * PRNG is re-created per call, so every run is byte-identical.
 */
export function seed(): { vendors: number; transactions: number; latestSpend: number } {
  ...
  return { vendors: SEED_VENDORS.length, transactions: txCount, latestSpend };
}

export const SEED_PERIODS = PERIODS.length;
