...
ok("backup recording exists (docs/backup-demo.webm|gif|mp4|mov)", ["webm", "gif", "mp4", "mov"].some((x) => existsSync(join(process.cwd(), "docs", `backup-demo.${x}`))), "the fallback is not optional — record it before judging (shot list in docs/DEMO_SCRIPT.md)");
